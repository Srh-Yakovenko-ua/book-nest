...
```ts
function normalizeQueuePriorityFields(values: QueuePriorityFormValues): QueuePriorityFormValues;
```
...
```ts
if (!values.queuePriorityReason || !REASONS_WITH_DATE.includes(values.queuePriorityReason)) {
...
```ts
import { ChevronsDown, ChevronsUp, Minus, type LucideIcon } from "lucide-react";
...
```ts
export const QUEUE_PRIORITY_CONFIG: Record<QueuePriority, PriorityConfig> = {
  low: {
    value: "low",
    label: "Низький",
  ...
  normal: {
    ...
    description: "Для книг, які плануєш читати у встановленому порядку.",
    extendedDescription:
  ...
  high: {
    ...
    description: "Для книг, які важливо прочитати найближчим часом.",
    extendedDescription:
