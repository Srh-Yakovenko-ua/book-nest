...
- **Strip-trap check:** grep pending migrations for `DROP INDEX`. NINE indexes are hand-written raw SQL and absent from `schema.prisma`, so generated migrations emit a spurious `DROP INDEX` for them — `authors_search_text_trgm_idx`, `publishers_search_text_trgm_idx` (trigram GIN, cross-locale search), `book_deliveries_active_book_idx`, `book_loans_active_book_idx` (partial-unique, one active delivery/loan per book), `books_user_queue_position_idx` (partial `WHERE queue_position IS NOT NULL`, reading-queue), `books_series_id_part_number_key` (partial-unique, so a trashed book releases its slot in the series — `book-relations-resolver` maps P2002 on that exact name to the friendly part-number error), and `series_user_id_normalized_name_key`, `book_lists_user_id_normalized_name_key`, `book_timelines_book_id_name_lower_idx` (partial-unique `WHERE deleted_at IS NULL`, so a trashed series/list/timeline releases its name). CLAUDE.md §6 is the canonical list — reread it rather than trusting this copy. If any pending migration drops one of these, it must be hand-stripped BEFORE deploy (dropping them silently kills cross-locale search, the one-active invariants, the queue index, or a uniqueness rule). `apps/api/src/core/database/raw-sql-indexes.test.ts` asserts all nine in CI, so a forgotten strip turns red there too. HALT and report if found.
