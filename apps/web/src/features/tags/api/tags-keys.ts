...
type TagFamily = "catalog" | "deletion-preview" | "facets" | "picker" | "selected" | "summary";
...
export const tagsKeys = {
  ...
  selected: (ids: readonly string[]) => [...family("selected"), ids] as const,
...
export async function invalidateTagPickerQueries(queryClient: QueryClient): Promise<void> {
  await Promise.all([
    queryClient.invalidateQueries({ queryKey: tagsKeys.pickers }),
    queryClient.invalidateQueries({ queryKey: tagsKeys.selections }),
  ]);
}
