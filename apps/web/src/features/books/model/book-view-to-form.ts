...
export function bookViewToFormState(book: BookView): BookFormInitialState {
  ...
  const values: CreateBookFormValues = {
    ...
    queuePriorityReason: book.queuePriorityReason ?? undefined,
    queuePriorityReasonCustomText: book.queuePriorityReasonCustomText ?? undefined,
    queuePriorityTargetDate: book.queuePriorityTargetDate ?? undefined,
...
function deliveryToInput(book: BookView): CreateBookFormValues["deliveryInfo"] {
  ...
  return {
    ...
    deliveryStatus: isActiveShipmentStatus(info.status) ? info.status : undefined,
