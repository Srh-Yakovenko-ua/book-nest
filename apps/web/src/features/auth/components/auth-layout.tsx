...
export function AuthLayout({ children, cover, tagline }: AuthLayoutProps) {
  return (
    <div className="min-h-svh bg-background min-[900px]:grid min-[900px]:grid-cols-2">
      <main className="relative flex min-h-svh flex-col overflow-y-auto px-4 pt-5 pb-8 min-[900px]:px-10 min-[900px]:pt-7 min-[900px]:pb-10 sm:px-[22px]">
        <header className="mb-1.5 flex items-center justify-between">
          <Logo variant="horizontal" />
          <ThemePicker triggerClassName="size-[42px] rounded-full border border-border bg-card text-foreground hover:bg-secondary [&_svg]:size-[18px]" />
        </header>

        <div className="flex flex-1 flex-col items-center justify-center py-[18px]">
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 p-11">
          ...
          <p className="mt-4 max-w-[18ch] font-heading text-[1.45rem] leading-snug text-white/90 italic">
            {tagline}
          </p>
        </div>
      </aside>
