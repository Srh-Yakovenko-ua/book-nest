...
import type { DedicationSort } from "@app/shared";
...
import { DEDICATION_SORT_OPTIONS } from "../model/dedications-query";

type DedicationsSortSheetProps = {
  className?: string;
  onChange: (value: DedicationSort) => void;
  value: DedicationSort;
};

export function DedicationsSortSheet({ className, onChange, value }: DedicationsSortSheetProps) {
  const t = useTranslations("dedications.sort");
  const tMobile = useTranslations("dedications.sort.mobile");
  const tToolbar = useTranslations("dedications.toolbar");
  const [open, setOpen] = useState(false);

  function handleSelect(next: string) {
    const option = DEDICATION_SORT_OPTIONS.find((candidate) => candidate === next);
        ...
        >
          {DEDICATION_SORT_OPTIONS.map((option) => (
            <Label
              ...
              htmlFor={`dedications-sort-${option}`}
              key={option}
            >
              <RadioGroupItem
                className="size-5 shrink-0"
                id={`dedications-sort-${option}`}
                value={option}
              />
              {t(option)}
            </Label>
