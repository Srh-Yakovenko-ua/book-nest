...
type LoanContactNameButtonProps = {
  ...
  name: string;
  onOpen: () => void;
} & ({ contact: Nullable<string>; layout: "block" } | { layout?: "inline" });

export function LoanContactNameButton(props: LoanContactNameButtonProps) {
  const t = useTranslations("loans.contactDrawer");
  const contactId = useId();

  if (props.layout === "block") {
    const { className, contact, name, onOpen } = props;
  ...
  const { className, name, onOpen } = props;
