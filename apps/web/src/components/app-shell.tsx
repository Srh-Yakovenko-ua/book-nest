...
type NavSectionChild = {
  labelKey: "history" | "inTransit" | "statistics";
  to: string;
};
...
const NAV_ITEMS = [
  ...
  { icon: HandHelping, key: "loans", kind: "link", to: "/loans" },
  {
    children: [
      { labelKey: "inTransit", to: "/delivery/in-transit" },
      { labelKey: "history", to: "/delivery/history" },
      { labelKey: "statistics", to: "/delivery/statistics" },
    ],
    icon: Truck,
    key: "delivery",
    kind: "section",
...
const NAV_STYLES = {
  ...
  subChild:
    "flex h-7 min-w-0 cursor-pointer items-center rounded-md px-2 transition-colors duration-150 outline-hidden focus-visible:ring-2 focus-visible:ring-sidebar-ring",
...
function NavSectionMenuItem({ item, pathname }: { item: NavSection; pathname: string }) {
  const tNav = useTranslations("nav");
  const tSection = useTranslations("delivery.subnav");
  ...
  return (
    ...
    >
      <SidebarMenuItem>
        ...
        <CollapsibleContent>
          <SidebarMenuSub aria-label={tSection("label")} className="mt-1">
              ...
              return (
                <SidebarMenuSubItem key={child.to}>
                  <Link
                    ...
                    href={child.to}
                  >
                    <span className={NAV_STYLES.subLabel}>{tSection(child.labelKey)}</span>
                  </Link>
                  {isChildActive && (
                    <motion.div
