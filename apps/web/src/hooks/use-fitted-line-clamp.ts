...
export function useFittedLineClamp<TElement extends HTMLElement>(
  ...
  const [lines, setLines] = useState<number>(TEXT_FIT.defaultLines);
...
function fitLines(element: HTMLElement, container: Nullable<HTMLElement>): number {
  if (container === null) return TEXT_FIT.defaultLines;
  if (!window.matchMedia(TEXT_FIT.from).matches) return TEXT_FIT.defaultLines;

  const lineHeight = Number.parseFloat(getComputedStyle(element).lineHeight);
  if (!Number.isFinite(lineHeight) || lineHeight <= 0) return TEXT_FIT.defaultLines;

  const freeSpace = freeSpaceIn(container);
  if (freeSpace === null) return TEXT_FIT.defaultLines;
...
function readFit(element: HTMLElement, container: Nullable<HTMLElement>): Fit {
  return {
    isClamped: element.scrollHeight > element.clientHeight + TEXT_FIT.overflowTolerancePx,
