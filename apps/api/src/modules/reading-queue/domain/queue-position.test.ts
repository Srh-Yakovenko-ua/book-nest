...
describe("computeQueueInsertPosition", () => {
  describe("placement end", () => {
    it("returns position 1 for an empty queue", () => {
      expect(computeQueueInsertPosition({ count: 0, maxPosition: 0, placement: "end" })).toEqual({
        ok: true,
        position: 1,
      });
    });

    it("returns maxPosition + 1 for a populated queue", () => {
      expect(computeQueueInsertPosition({ count: 3, maxPosition: 3, placement: "end" })).toEqual({
        ok: true,
        position: 4,
      });
    });
  ...
  describe("placement start", () => {
    it("returns position 1 for an empty queue", () => {
      expect(computeQueueInsertPosition({ count: 0, maxPosition: 0, placement: "start" })).toEqual({
        ok: true,
        position: 1,
      });
    });
    ...
    it("returns position 1 regardless of the queue length", () => {
      expect(computeQueueInsertPosition({ count: 3, maxPosition: 3, placement: "start" })).toEqual({
        ok: true,
        position: 1,
      });
    });
