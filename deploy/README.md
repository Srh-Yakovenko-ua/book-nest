...
1. **renders `~/booknest/.env`** from **GitHub repo Secrets** (`DEV/PROD_JWT_*`, `DEV/PROD_POSTGRES_PASSWORD`, `PROD_SMTP_PASS`, `MAILPIT_BASIC_AUTH_HASH`) plus non-secret constants, and `scp`s it over;
...
- `Caddyfile` — routing + TLS + the Mailpit basic-auth. Synced from repo by CI. The basic-auth hash comes from `MAILPIT_BASIC_AUTH_HASH`; generate one with `docker run --rm caddy:2-alpine caddy hash-password --plaintext '<password>'`. Caddy refuses to load its whole config when it is unset, so the deploy job hard-fails before touching the server if the secret is missing.
